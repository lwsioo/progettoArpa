# 🦊 Fauna in Città

![GitHub repo size](https://img.shields.io/github/repo-size/lwsioo/progettoArpa?style=for-the-badge)
![GitHub last commit](https://img.shields.io/github/last-commit/lwsioo/progettoArpa?style=for-the-badge)
![GitHub issues](https://img.shields.io/github/issues/lwsioo/progettoArpa?style=for-the-badge)

Un progetto sviluppato per **ARPA Umbria** con l'obiettivo di sensibilizzare la cittadinanza sulla presenza e la tutela della fauna selvatica in ambienti urbani. 🏙️🌿

🔗 **Sito web ufficiale:** [Fauna in Città](https://apps.arpa.umbria.it/faunaincitta/home.html)

---

## ✨ Funzionalità
✅ Mappa interattiva della fauna selvatica
✅ Materiali didattici e informativi
✅ Accessibilità per utenti ipovedenti
✅ Collaborazione con enti e associazioni locali

---

## 📡 Come Funziona
Il progetto raccoglie e diffonde informazioni sulla fauna urbana, promuovendo buone pratiche per la coesistenza tra cittadini e animali selvatici. 

- **Piattaforma Web** sviluppata per la consultazione pubblica
- **Materiale Didattico** disponibile per educatori e studenti
- **Eventi e Attività** per la sensibilizzazione e il coinvolgimento della comunità

---

## 🛠️ Tecnologie Utilizzate
- JavaScript 🌐
- HTML/CSS 🎨
- Leaflet.js 🗺️ (per la mappa interattiva)

---

## 📜 Licenza
Questo progetto è distribuito sotto licenza MIT. 📄

---

💡 **Star il repository se ti è piaciuto! ⭐**


